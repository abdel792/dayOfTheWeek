...
# this technique will be improved if other languages ??encounter the same problem.
...
def isDatepickerDate(value):
	"""
	This function makes it possible to check
	whether the date format matches to be able to affect our overlay class.
	"""
...
class DateDialog (wx.Dialog):
	...
	def __init__(self, parent):
		...
		# which included the gui.guiHelper module.
		if not hasattr(gui, "guiHelper"):
			self.showDateDialog()
		else:
...
class DayOfWeekSettingsDialog (SettingsDialog):
	...
	def makeSettings(self, settingsSizer):
		...
		# Translators: The label for an item to select the level of the announces of labels.
...
class AnnounceFieldsLabels (IAccessible):
	...
	def whatChanged(self, val1, val2):  # noqa: C901
		...
		if val1[0] != val2[0]:
			...
			if self.isUS() and len(val1[2]) == 4 and len(val1[1]) < 3:
				...
				# Since the calculation function executes
				# a down arrow and an up arrow to find the value of the current month,
				# it often switches to shorter months, which changes the value of the day.
				...
				if val2[1] == "28":
					if val1[1] == "31":
						...
						keyboardHandler.KeyboardInputGesture.fromName("leftArrow").send()
						api.processPendingEvents()
				# The following elif block is for the leap years,
				# where the February month is 29 days.
				elif val2[1] == "29":
					if val1[1] == "31":
						keyboardHandler.KeyboardInputGesture.fromName("rightArrow").send()
						keyboardHandler.KeyboardInputGesture.fromName("upArrow").send()
				...
				if len(curValue) == 4:
					...
					# Here is a technique to fix the problem when switching to shorter years,
					# for instance, when switching from a leap year to a common year.
					# Since the calculation function executes
					# a down arrow and an up arrow to find the value of the current year,
					# it sometimes switches to shorter years, which changes the value of the day field in February month.
					if val2[2] == "28":
					...
					if any(x == "28" for x in [val2[0], val2[1]]):
						if self.isUS():
							...
							if self.isUS():
								...
								keyboardHandler.KeyboardInputGesture.fromName("leftArrow").send()
							api.processPendingEvents()
		# The following if statement has been added to correct the current field's
		# non-recognition bug when year is 1601.
		if curValue == "1601":
			curDateField = 3
	...
	def calculateCurField(self):
		val1 = self.value
		# The following calculation must be made in all cases,
		# to allow those who use only Braille, to have the announcement of the labels of the date fields.
